...
class Metadata:
    labels = dict(
        ...
        ghg_no_lucf_range=("Total GHG Emissions excluding LUCF MtCO2e 2014 ranges"),
    ...
    def _get_value(target, name):
        ...
        return [v.name for v in value.all()] if hasattr(value, "all") else value

    def _get_label(self, name):
        return self.labels[name] if name in self.labels else self.form[name].label
    ...
    def __getattr__(self, name):
        ...
        val_custom = (
            self._get_value(self.customised, name) if self.is_customised() else val_orig
        )
...
class Details(DetailView):
    ...
    def get_object(self):
        ...
        return get_object_or_404(self.model, iso=iso)
    ...
    def get_context_data(self, **kwargs):
        ...
        try:
            metadata_user = AssessmentProfile.objects.get(
                country__iso=self.object.iso, user=self.request.user_profile
            )
...
def _get_user_metadata(iso, user_profile):
    try:
        meta = AssessmentProfile.objects.get(country__iso=iso, user=user_profile)
    except AssessmentProfile.DoesNotExist:
    ...
    def get_context_data(self, **kwargs):
        ...
        try:
            metadata_user = self.model.objects.get(
                country__iso=self.object.country.iso, user=self.request.user_profile
            )
...
class DeleteCustomisedProfile(DeleteView):
    ...
    def get_success_url(self, **kwargs):
        iso = self.kwargs.get(self.pk_url_kwarg)
        return reverse("lcc:country:view", kwargs={"iso": iso})

    def get_object(self):
        iso = self.kwargs.get(self.pk_url_kwarg)
        return AssessmentProfile.objects.get(
            user=self.request.user_profile, country__iso=iso
        )
