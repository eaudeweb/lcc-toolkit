    ...
    def clean_year_mention(self):
        ...
        if year_mention:
            years_in_year_mention = [
                int(year) for year in re.findall("\d\d\d\d", year_mention)
            ]

            if years_in_year_mention:
                if not any(
                    year in LEGISLATION_YEAR_RANGE for year in years_in_year_mention
                ):
                    self.add_error(
                        "year_mention",
...
class RegisterForm(ModelForm):
    ...
    role = forms.ChoiceField(
        label="Desired role",
        choices=map(lambda x: (x, x), roles.get_nonprivileged_roles()),
    )
    ...
    def save(self, commit=False):
        ...
        user = models.User.objects.create_user(
            email, email=email, first_name=first_name, last_name=last_name
        )
...
class ApproveRegistration(ModelForm):
    role = forms.ChoiceField(
        label="Desired role",
        choices=map(lambda x: (x, x), RolesManager.get_roles_names()),
    )
...
class CountryBase(ModelForm):
    ...
    def others(self):
        return (
            field
            for field in (self[name] for name in self.fields)
