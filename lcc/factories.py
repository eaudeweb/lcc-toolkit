...
class LegislationFactory(factory.django.DjangoModelFactory):
    ...
    language = factory.fuzzy.FuzzyChoice([tup[0] for tup in ALL_LANGUAGES])
    law_type = factory.fuzzy.FuzzyChoice([tup[0] for tup in LEGISLATION_TYPE])
    ...
    year = factory.LazyAttribute(
        lambda obj: (obj.year_amendment - factory.fuzzy.FuzzyChoice(range(10)).fuzz())
    )

    geo_coverage = factory.fuzzy.FuzzyChoice([tup[0] for tup in GEOGRAPHICAL_COVERAGE])
