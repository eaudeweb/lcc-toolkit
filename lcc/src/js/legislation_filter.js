...
function activatePagination() {
    ...
    $("ul.pagination li a").on('click', function() {
        Object.keys(orderByOptions).map((orderKey) => {
            const queryMatch = window.location.search.match(`\A?${orderKey}=[^&]*`);
...
function setOrderBy() {
    ...
    const pageQueryMatch = window.location.search.match(`\A?page=[^&]*`);
    const pageNumber = pageQueryMatch ? pageQueryMatch[0].split('=')[1] : null;
    ...
    $("ul.dropdown-menu-order li a").on('click', function() {
        const orderName = $(this).attr('data-sort-id');
        const orderValue = $(this).attr('data-sort-dir');
